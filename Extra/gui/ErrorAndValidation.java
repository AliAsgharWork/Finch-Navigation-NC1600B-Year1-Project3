...
public class ErrorAndValidation  {
	...
	public boolean counting() {
		...
		if (countCon < timeCon)
		{
			...
			System.out.println("Current Sucessful Stored Executions: "+ countCon);
			return false;
		}
		return true;
	}
	...
	public static boolean input_char_validacy(String [] Array_input) {
		...
		for (int i = 0; i < Array_input.length; i++) 
		{
			for (int k = 0; k < Array_input[i].length(); k++) 
			{
				...
				{
					System.err.println("The command inputed contains illegal characters:"+Array_input[i].charAt(k));
					prob = false;
				}	
			}
		}
	...
	private boolean ortogonal_handling() {//CHANGE VARIABLE NAMES TO SOMETHING APPROPIRATE (PT,pL,etc...)
		...
		if(commandCon.equals("R")){//Assigning values according to the time condition for RIGHT turn.
			...
			if(timeCon==6) {T=6;for6positive=13;  for6negetive=for6positive*-13;} //EXCEPTIONAL CASE
	...
	private boolean speedRange() 
	{
		...
		if(commandCon.equals("F")&&(rightCon>100 || rightCon<-100))
		{
	...
	private static boolean array_Lenght() {
		...
		{System.out.println("The entered command for \""+commandCon+"\" should have 3 inputs (Command, Time and Velocity)");return false;}
		if((commandCon.equals("R")||commandCon.equals("L"))&&array_inputCon.length!=4) 
		{System.out.println("The entered command for \""+commandCon+"\" should have 4 inputs (Command, Time, Velocity for Right Wheel and Velocity for Left Wheel )");return false;}
